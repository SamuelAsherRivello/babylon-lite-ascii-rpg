...
import { getCharacterSlotPresentation, INITIAL_CHARACTER } from "./character-data.js";
...
const GOLD_COIN_ASSET = `${import.meta.env.BASE_URL}assets/images/Dungeons-and-Pixels-v1.4/Items/Animated/gold_coin.png`;
...
export function CharacterDetails({ gold = INITIAL_CHARACTER.gold.currentAmount, keys = INITIAL_CHARACTER.keys.currentAmount, health = INITIAL_CHARACTER.health.currentPercent, stamina = INITIAL_CHARACTER.stamina, combatStats = { offense: INITIAL_CHARACTER.offense, defense: INITIAL_CHARACTER.defense }, experience = INITIAL_CHARACTER.experience, slots = INITIAL_CHARACTER.slots, palette, onShowTooltip, onHideTooltip }) {
  ...
  const slot = (item, index) => { const presentation = getCharacterSlotPresentation(item, SLOT_LABELS[index]); return <Tooltip description={item?.name ?? SLOT_LABELS[index]} onShow={onShowTooltip} onHide={onHideTooltip}><div className="character_resource character_slot" aria-label={SLOT_LABELS[index]}><span className={`character_slot_text${item ? " character_item_icon" : ""}`}>{presentation.glyph}</span>{presentation.count !== null ? <span className="character_resource_value" data-item-count="bomb">{presentation.count}</span> : presentation.showHealth ? <ItemHealthBar item={item} /> : null}</div></Tooltip>; };
  return <div className="character_details" aria-label="Character details"><div className="character_bar_container">{ROWS.map((row) => <Bar key={row.key} row={row} data={characterData[row.key]} onShowTooltip={onShowTooltip} onHide={onHideTooltip} />)}</div><div className="character_slots_container"><div className="character_slots_grid"><Tooltip description="Gold: The currency of your character." onShow={onShowTooltip} onHide={onHideTooltip}><div className="character_resource" data-resource="gold" aria-label="Gold"><img className="character_resource_icon character_gold_coin_icon" src={GOLD_COIN_ASSET} alt="" aria-hidden="true" /><span className="character_resource_value">{gold}</span></div></Tooltip>{slots.slice(0, 2).map(slot)}<Tooltip description="Keys: The keys your character is holding." onShow={onShowTooltip} onHide={onHideTooltip}><div className="character_resource" data-resource="keys" aria-label="Keys"><span className="character_resource_icon" aria-hidden="true" style={{ color: keyStyle.color }}>⚿</span><span className="character_resource_value">{keys}</span></div></Tooltip>{slots.slice(2, 4).map((item, index) => slot(item, index + 2))}</div></div></div>;
}
