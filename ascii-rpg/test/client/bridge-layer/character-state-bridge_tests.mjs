...
test("publishes immutable character-state snapshots through the bridge", () => {
  ...
  sendCharacterStateSnapshot({
    slots: [{ slot: "Slot 01", id: "sword", glyph: "🗡", name: "Sword", health: 1000, maxHealth: 1000 }, null, null, { slot: "Slot 04", id: "bomb", glyph: "●", name: "Bomb", count: 49 }],
    gold: 2,
    keys: 1,
  });
  stop();
  ...
  assert.equal(snapshot.slots[3].count, 49);
