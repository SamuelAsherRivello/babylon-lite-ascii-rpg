...
test("creates every visible Code Page 437 entry, the bullet, and civilization text symbols", () => {
  ...
  assert.equal(palette.length, 298);
  assert.equal(ids.size, 298);
  assert.equal(new Set(palette.map((entry) => entry.glyph)).size, 298);
  assert.ok(ids.has("32"));
  assert.ok(ids.has("254"));
  assert.ok(ids.has("U+2022"));
  for (const id of ["U+2191", "U+2665", "U+25C7", "U+266A", "U+2694", "U+25AC", "U+25AD", "U+25AE", "U+25AF", "U+26BF"]) {
...
test("serializes a complete validated palette", () => {
  ...
  assert.equal(serialized.entries.length, 298);
});
