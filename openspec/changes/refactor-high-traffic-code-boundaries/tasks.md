...
- [ ] 2.6 Reduce `world-system.js` to stable world-state and public-facade responsibilities; verify all world-system tests and existing imports remain valid.
...
- [ ] 4.1 Extract game-service construction and lifecycle disposal into a `game-session` factory while retaining `startGameLayer` as the existing facade; verify game startup, restart, and disposal tests pass.
...
- [ ] 6.2 Run `npm.cmd run build`; verify the GitHub Pages production bundle completes without new dependencies.
