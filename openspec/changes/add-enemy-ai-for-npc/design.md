...
- **Enemy cadence remains player-driven logical time.** Enemies receive one eligible action every two world-time ticks, while an idle player advances no time and therefore triggers no enemy action. This preserves the existing turn-based pacing without a wall-clock timer.
