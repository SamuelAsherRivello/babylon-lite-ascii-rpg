...
The ASCII Palette SHALL retain entries for uppercase `E` and uppercase `S` and SHALL assign both an explicit red base color for Enemy and Enemy Spawner rendering. Client lighting MAY adjust visible brightness, but enemy and spawner identity SHALL remain palette-driven.
