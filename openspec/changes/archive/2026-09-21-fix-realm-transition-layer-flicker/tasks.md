...
- [ ] 1.2 Reproduce both stair and Settings realm-transfer paths through the seam, and verify the current regression is represented by an observable detach/attach or blank-presentation ordering.
...
- [x] 3.1 Verify paired-stair arrival, realm bridge status, input locking, fog isolation, and React UI stacking remain unchanged while the new renderer-continuity assertions pass.
- [x] 3.2 Run the focused transition and game-layer tests, then run `npm.cmd test` and `npm.cmd run build`; record any environment-limited validation explicitly.
