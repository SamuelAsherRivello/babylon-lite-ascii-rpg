...
- Make editor previews and runtime rendering use one offset-aware rasterization path.
...
- Emoji metrics vary by platform font -> preview and runtime must share the active font path, but exact emoji artwork may still differ across browsers or operating systems.
...
3. Runtime loading of old checked-in or localStorage palettes should produce entries with zero offsets and unchanged visible rendering.
