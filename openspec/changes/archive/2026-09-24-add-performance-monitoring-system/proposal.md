...
- After each requested scan, append a concise result entry of 10–100 words to
  `output/performance-analysis/performance-monitoring.md`. Each entry SHALL include
  the date, scan scenarios, headline measurements, and any optimization finding
  or explicit “no candidate identified” result; raw reports remain transient.
...
- The durable Markdown log at `output/performance-analysis/performance-monitoring.md`
  is the human-readable project record; it is not a
  replacement for structured transient reports and SHALL not contain secrets,
  storage contents, seeds, or unrelated gameplay state.
