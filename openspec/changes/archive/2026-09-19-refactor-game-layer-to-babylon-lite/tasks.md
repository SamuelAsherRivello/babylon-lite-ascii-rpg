...
- [x] 1.3 Refactor `main.jsx` so React mounts only into `ui_layer` and the
  Babylon Lite game bootstrap mounts into `game_layer`; verify startup code no
  longer renders a React game component.
- [x] 1.4 Define the game controller/bridge surface for deliberate UI commands
...
- [x] 2.2 Move viewport sizing and resize handling into the Babylon Lite game
  layer and verify full-viewport logical grid sizing remains correct.
- [x] 2.3 Move keyboard input, held-key state, initial repeat delay, repeat
...
- [x] 2.4 Move world startup, `?randomSeed=value` consumption, player start
...
- [x] 3.1 Implement Babylon Lite ASCII world rendering for visible cells and
  verify `W`, `•`, and `P` render with character-over-terrain precedence.
- [x] 3.2 Apply active palette color and alpha inside Babylon Lite rendering
  and verify mixed-style glyphs render independently.
- [x] 3.3 Keep the Ascii Palette window, color picker, alpha control,
  Confirm/Cancel, warnings, and persistence interactions in React and verify
  palette UI tests still pass.
- [x] 3.4 Connect confirmed React palette snapshots to the Babylon Lite bridge
  and verify changing `•` and `W` updates the live game world without React
  rendering any world glyph cells.
- [x] 3.5 Connect Arguments UI/startup argument documentation to Babylon Lite
...
- [x] 4.2 Remove or demote old canvas rendering helpers to tests-only fixtures
  where still useful and verify production imports do not depend on the legacy
  renderer.
- [x] 4.3 Ensure Babylon Lite/WebGPU initialization failure does not start the
  legacy canvas gameplay fallback and verify the game world remains unloaded in
  that failure path.

## 5. Verification

- [x] 5.1 Update focused unit tests for pure world generation, movement rules,
  palette validation, and bridge contracts; verify `npm.cmd test` passes.
- [x] 5.2 Run `npm.cmd run build` and verify the production bundle succeeds.
- [x] 5.3 Run the Vite app and verify in a real browser that Babylon Lite
  ...
  edits from React.
- [x] 5.4 Validate the OpenSpec change with
