...
When gameplay input is available, a non-repeating SPACE key press SHALL request the `bomb` capability through the existing inventory action-resolution system. If the character has at least one bomb and no planted bomb already occupies the player's current cell, the game SHALL plant one bomb on that cell and decrement the stack by exactly one. Failed placement SHALL not consume a bomb. A planted bomb SHALL not claim exclusive character occupancy or block actor movement. A planted bomb SHALL render with the `●` glyph and remain at its planted cell after the player leaves.

#### Scenario: Place bomb at the current cell
- **WHEN** the player presses SPACE with a positive bomb count and no bomb at the player's cell
- **THEN** one bomb SHALL appear at the player's current cell and the bomb count SHALL decrease by one
...
- **WHEN** the player presses SPACE while a planted bomb already occupies the player's cell
...
- **THEN** the reached bomb SHALL begin its own radius-one explosion at time 21
