...
- Never restore or render the legacy Trap glyph, including when the artwork is unavailable, and preserve all existing Trap generation, movement, collision, health-consequence, logging, minimap, and preview behavior.
...
- `procedural-level-generation`: Define the visible in-world animated Trap artwork, its continuous loop behavior, fog/region eligibility, and no-glyph-fallback behavior while retaining the existing static-object and gameplay contracts.
