...
- Implement the first animation as a soft-edged, centered red iris with two
  2-second phases and a safe midpoint for realm replacement.
...
canvases, and drive its center, radius, and feather values from the Babylon
controller. A radial gradient gives the requested soft edge, follows the
...
types.
The iris radius starts at a viewport-covering value, reaches a radius derived
from the visible player-cell footprint, and then grows back to the covering
value. The feather width is derived from the radius with a minimum pixel
floor, so the edge remains visible at both desktop and mobile scales.
...
arrival and rendering work while covered, then resumes input only after the
opening phase completes.
