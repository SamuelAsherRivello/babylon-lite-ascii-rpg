...
- [ ] 4.1 Add focused Node tests for dialog state transitions, cardinal collision entry, repeatable signs, NPC recruitment, and recruited passability, and verify they pass from the repository root.
- [ ] 4.2 Run `npm.cmd test` and `npm.cmd run build` from the repository root and verify no unrelated worktree files are modified.
