...
After a requested monitoring scan completes, the client/project workflow SHALL
append one human-readable result entry of between `10` and `100` words to
`output/performance-analysis/performance-monitoring.md`. The entry SHALL include
the date, scenarios measured, headline FPS/readiness/render findings, and an
optimization conclusion or explicit statement that no candidate was identified.
Raw frame samples and structured reports SHALL remain transient unless a later
request explicitly adds another storage format.
...
#### Scenario: Scan result is recorded in the project log

- **WHEN** a developer completes a requested performance scan
- **THEN** one dated 10–100-word result entry SHALL be appended to
  `output/performance-analysis/performance-monitoring.md` with the measured
  scenarios, headline results, and optimization conclusion
