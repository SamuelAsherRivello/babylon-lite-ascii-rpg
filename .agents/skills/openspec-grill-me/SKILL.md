...
This is a custom OpenSpec companion skill for structured planning interviews. It is intentionally optional and is not part of OpenSpec's core profile.
...
## Fold Results Into the Active OpenSpec Phase

At the end of the interview, fold the confirmed decisions and unresolved
items into the workflow phase that is actually underway:

1. If the explorer phase is underway, update the exploration output or
   exploration notes with the clarified problem, decisions, evidence,
   tradeoffs, and remaining questions.
2. If the proposal phase is underway, incorporate the conclusions into the
   proposal's scope, requirements, design, tasks, and explicit open items.
3. If the artifacts are already complete, reconcile the conclusions directly
   into the owning artifacts, keeping all affected artifacts consistent.

Determine the phase from the target's actual status and existing files; do not
create a parallel summary when the active phase has an appropriate artifact.
Preserve unresolved decisions explicitly and do not advance the workflow to
the next phase unless the user asked for that transition.
